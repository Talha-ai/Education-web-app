...
const CoursesPage = () => {
  ...
  const fetchEducatorName = async (educatorId) => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/educator/${educatorId}/profile`
      );
    ...
    <div className="my-10 mx-5">
      ...
      <div className="grid gap-10 justify-center items-center grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {courses.map((course, index) => (
          <div
            key={course._id}
            className="shadow-custom rounded-lg hover:scale-95 transition-all"
          >
            <Link
              to={`/course/${course._id}`}
              className="block bg-[#f4f4f4] p-1 rounded-b-lg"
            >
              <img
